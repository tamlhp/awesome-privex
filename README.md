...
|                        | Faithfulness (RDT-Fidelity) | $ \mathcal{F}(\mathcal{E}_X) = \mathbb{E}  $ | <div style="width:500px"> Reflect how often the model's predictions are unchanged despite perturbations to the input, which would suggest that the explanation is effectively capturing the reasoning behind the model's predictions.    </div>          |
